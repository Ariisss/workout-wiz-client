...
export default function Dashboard({}: Props) {
  ...
  const { completedDays, totalDays } = getWeeklyCompletion(
    dashboardData.dailyExercises
  );
  ...
  return (
    <div className="h-fit w-full flex flex-col gap-8 py-8 pl-8 md:pl-16 pr-8">
      <div>
        <ValueContent
          main={`Welcome back, ${userData?.username}`}
          sub={"Here's your fitness overview for today"}
        />
      </div>
      <div className="min-h-[144px] flex flex-col lg:flex-row gap-6">
        <DashboardCard
          ...
          icon={
            <Dumbbell className="-rotate-45 text-primary-light h-6 w-6 my-[-5px]" />
          }
        >
          <ValueContent
            main={dashboardData.workoutStats.total.toString()}
            sub={`+${dashboardData.workoutStats.lastWeek} from last week`}
          />
        </DashboardCard>
        <DashboardCard
          subHeader
        ...
        >
          ...
          userData?.weeklyStreak === undefined ? (
            <ValueContent
              main="No streak"
              sub="Get moving to start your streak!"
            />
          ) : (
            <ValueContent
              main={
                userData.weeklyStreak.toString() +
                " week" +
                (userData.weeklyStreak > 1 ? "s" : "")
              }
              sub="Fuelling the progress machine!"
            />
          )}
        </DashboardCard>
      </div>
      <div className="lg:h-[350px] flex flex-col lg:flex-row gap-6">
        ...
        <DashboardCard
          ...
          desc={
            totalDays == 0
              ? "No workouts yet"
              : `${completedDays} out of ${totalDays} workouts completed`
          }
